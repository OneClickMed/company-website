...
function getKeywords(article: BlogArticle | null): string[] | undefined {
  ...
  const base = ['OneClickMed', 'healthcare resources', 'digital health']
...
export async function generateMetadata({ params }: ResourceDetailPageProps): Promise<Metadata> {
  ...
  const fallbackDescription = `Explore this ${siteName} resource on digital health and patient care.`
...
export default async function ResourceDetailPage({ params, searchParams }: ResourceDetailPageProps) {
  ...
  const description = getDescription(
    article,
    `Explore this ${siteName} resource on digital health and patient care.`
  )
  const publishedTime = getPublishedISO(article?.createdAt)
  const jsonLd =
    article && !isAppView
